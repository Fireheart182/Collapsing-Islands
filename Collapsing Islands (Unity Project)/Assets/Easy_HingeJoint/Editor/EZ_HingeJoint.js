						...
						for(var dero : Vector3 in findery.GetComponent(ArrayVector3).ArrayVector3)
						{
							if(Physics.Raycast(obj.transform.position, dero , hit,distance))
							{
								if(hit.collider.gameObject)
								{
									...
									addy.connectedBody = hit.collider.gameObject.GetComponent.<Rigidbody>();
									addy.anchor = Anchor;
									addy.axis = Axis;
									...
									addy.limits.min = min;
									addy.limits.bounciness = minBounce;//PROBLEM IS HERE 
									//addy.limits.max = max;
									addy.limits.bounciness = maxBounce;
							...
							for(var colld : Collider in ColiAll)
							{
								if(colld.transform != obj.transform)
								{
									if(colld)
									{
										...
										addyy.connectedBody = colld.gameObject.GetComponent.<Rigidbody>();
										addyy.anchor = Anchor;
										addyy.axis = Axis;
										...
										addyy.limits.bounciness = minBounce;
										//addyy.limits.max = max;
										addyy.limits.bounciness = maxBounce;
